...
The next time someone visits the app from the device, the UI loads from the cache immediately and any new content is requested from the server (if it isn’t available in the cache already).
...
<CodeSurfer
  ...
  dark={false}
  steps={[
    { range: [22,29], notes: " URL Patterns for caching images from the cat API ✏️" },
  ]}
/>
...
SW precache svg for offline messages and images from external resources
...
## ✨Bonus Nugget ! ✨ hawt arrow functions 🔥➡️
...
**USE CASE** - new content alert, offline warning alert
